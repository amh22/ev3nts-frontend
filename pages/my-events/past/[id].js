...
function PastEvent() {
  ...
  const confirmAttendee = async (attendee) => {
    try {
      ...
      if (ev3ntsContract) {
        const txn = await ev3ntsContract.confirmAttendee(event.id, attendee)
        setLoading(true)
  ...
  const confirmAllAttendees = async () => {
    ...
    try {
      ...
      if (ev3ntsContract) {
        ...
        const txn = await ev3ntsContract.confirmAllAttendees(event.id, {
          gasLimit: 300000,
        })
  ...
  return (
    mounted && (
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
        <Head>
          <title>My Dashboard | Ev3nts</title>
              ...
              account.address.toLowerCase() === event.eventOwner.toLowerCase() ? (
                <section>
                  <Link href='/my-events/past'>
                    <a className='text-indigo-800 text-sm hover:underline'>&#8592; Back</a>
                  </Link>
                                ...
                                <td className='relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6'>
                                  {checkIfConfirmed(event, rsvp.attendee.id) ? (
                                    ...
                                    <button
                                      ...
                                      onClick={() => confirmAttendee(rsvp.attendee.id)}
                                    >
                                      Confirm attendee
                                    </button>
                                  )}
