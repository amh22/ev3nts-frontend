...
function Event({ event }) {
  ...
  const newRSVP = async () => {
    try {
      ...
      if (ev3ntsContract) {
        const txn = await ev3ntsContract.createNewRSVP(event.id, {
          value: event.deposit,
          gasLimit: 300000,
        })
        setLoading(true)
        ...
        <div className='flex flex-wrap-reverse lg:flex-nowrap'>
          ...
          <div className='max-w-xs w-full flex flex-col gap-4 mb-6 lg:mb-0'>
            ...
            <div className='flex items-center'>
              ...
              <span className='truncate'>
                Hosted by{' '}
                <a className='text-indigo-800 truncate hover:underline' href={''} target='_blank' rel='noreferrer'>
                  {event.eventOwner}
                </a>
              </span>
            </div>
